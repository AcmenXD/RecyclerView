...
public abstract class OnSwipeMenuListener {
     ...
     * * 如不想添加菜单, 返回null即可
     */
    public abstract int[] getLeftMenuLayoutIds(int dataPosition);
     ...
     * * 如不想添加菜单, 返回null即可
     */
    public abstract int[] getRightMenuLayoutIds(int dataPosition);

    /**
     * 菜单项的单击回调事件
     *
     * @param dataPosition     定位数据的position
     * @param menuItemLayoutId 被点击的菜单项Id
     * @param direction        SwipeMenuView.LEFT_DIRECTION:左侧菜单  SwipeMenuView.RIGHT_DIRECTION:右侧菜单
     * @return true:点击后关闭菜单项 false:点击后不关闭菜单项
     */
    public abstract boolean onMenuItemClick(int dataPosition, int menuItemLayoutId, int direction);
}
